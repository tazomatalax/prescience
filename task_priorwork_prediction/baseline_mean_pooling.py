"""Mean pooling baseline for prior work prediction. Aggregates author embeddings via mean pooling of publications."""
...
def create_author_embedding(author_id, cutoff_date, num_recent_papers, sd2publications, all_papers_dict, all_embeddings):
    """Create author embedding from their recent publications (mean pooled, L2 normalized)."""
    ...
    embeddings = [get_embedding(all_embeddings, cid) for cid in recent_pubs if cid in all_embeddings]
...
def predict_references(author_ids, cutoff_date, num_recent_papers, k, sd2publications, all_papers_dict, all_embeddings, candidate_paper_ids):
    """Predict references by similarity of aggregated author embedding to candidate papers."""
    # Compute embedding for each author with publications
    author_embeddings = []
    for author_id in author_ids:
        emb = create_author_embedding(author_id, cutoff_date, num_recent_papers, sd2publications, all_papers_dict, all_embeddings)
        if emb is not None:
    ...
    valid_candidates = [cid for cid in candidate_paper_ids if cid in all_embeddings]
    if not valid_candidates:
        return [], []

    candidate_embeddings = np.stack([get_embedding(all_embeddings, cid) for cid in valid_candidates])
...
def main():
    parser = argparse.ArgumentParser(description="Mean pooling baseline for prior work prediction")
    ...
    output_path = os.path.join(args.output_dir, f"predictions.mean_pooling.{args.embedding_type}.json")
        ...
        if corpus_id in eval_instance_dict:
            ...
            if idx in selected_indices:
                ...
                predicted_ids, predicted_scores = predict_references(
                    ...
                    sd2publications, all_papers_dict, all_embeddings, candidate_paper_ids
                )
