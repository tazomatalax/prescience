"""Evaluation script for coauthor prediction task."""
...
def evaluate_predictions(predictions):
    ...
    for pred in tqdm(predictions, desc="Evaluating predictions"):
        ...
        predicted_coauthor_ids = pred["predicted_coauthor_ids"]

        ndcg = utils.calculate_ndcg(predicted_coauthor_ids, gt_coauthor_ids)
        k = len(gt_coauthor_ids)
        top_k_predictions = predicted_coauthor_ids[:k]
        precision, recall, f1 = utils.calculate_precision_recall_f1(top_k_predictions, gt_coauthor_ids)
        ...
        results.append({
            ...
            "recall": recall,
            "f1": f1,
            "num_gt_coauthors": len(gt_coauthor_ids),
        })
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate coauthor predictions")
    parser.add_argument("--predictions_path", type=str, required=True, help="Path to predictions JSON file")
    parser.add_argument("--output_dir", type=str, default="data/task_coauthor_prediction/test/scored")
