"""Mean pooling baseline for coauthor prediction. Computes author embeddings via mean pooling of paper embeddings."""
...
def create_author_embedding(paper_corpus_ids, all_embeddings):
    """Create L2-normalized author embedding via mean pooling of paper embeddings."""
    if not paper_corpus_ids:
        return None
    embeddings = [get_embedding(all_embeddings, cid) for cid in paper_corpus_ids]
    mean_emb = np.mean(embeddings, axis=0)
    return l2_normalize(mean_emb)


def compute_all_author_embeddings(cutoff_date, num_recent_papers, sd2publications, all_papers_dict, all_embeddings):
    """Pre-compute embeddings for all authors with publications before cutoff_date."""
    author_embeddings = {}
    ...
    for author_id, pub_ids in tqdm(preexisting_pubs.items(), desc="Creating author embeddings"):
        if len(pub_ids) > 0:
            ...
            emb = create_author_embedding(recent_pubs, all_embeddings)
            if emb is not None:
                author_embeddings[author_id] = emb

    utils.log(f"Created embeddings for {len(author_embeddings)} authors")
    return author_embeddings


def update_author_embedding(author_embeddings, author_id, pub_corpus_ids, all_embeddings):
    """Update an author's embedding."""
    if len(pub_corpus_ids) > 0:
        emb = create_author_embedding(pub_corpus_ids, all_embeddings)
        if emb is not None:
            author_embeddings[author_id] = emb
    return author_embeddings


def predict_coauthors(first_author_id, num_recent_papers, k, cutoff_date, sd2publications, all_papers_dict, all_embeddings, author_embeddings, all_author_ids, excluded_ids=None):
    """Predict coauthors by cosine similarity between query author and all candidates."""
    if excluded_ids is None:
        excluded_ids = set()
    excluded_ids = excluded_ids | {first_author_id}

    # Get query author's embedding from their recent papers
    ...
    query_embedding = create_author_embedding(recent_pubs, all_embeddings)

    if query_embedding is None:
        return [], []

    # Get all candidate authors (those with embeddings, excluding query)
...
def main():
    parser = argparse.ArgumentParser(description="Mean pooling baseline for coauthor prediction")
    ...
    output_path = os.path.join(args.output_dir, f"predictions.mean_pooling.{args.embedding_type}.{args.seed_author_type}.json")
    ...
    author_embeddings = compute_all_author_embeddings(first_date, args.num_recent_papers, sd2publications, all_papers_dict, all_embeddings)

    # Filter to papers on or after first_date
    postdated_papers = [p for p in all_papers if p["date"] >= first_date]

    utils.log(f"Running mean pooling baseline with embedding: {args.embedding_type}")
    predictions = []
    for paper in tqdm(postdated_papers, desc="Running mean pooling baseline"):
        ...
        if "authors" in paper:
            for author in paper["authors"]:
                ...
                author_embeddings = update_author_embedding(author_embeddings, author_id, recent_pubs, all_embeddings)
        ...
        if corpus_id in eval_instance_dict:
            ...
            if idx in selected_indices:
                ...
                pred_ids, pred_scores = predict_coauthors(
                    ...
                    sd2publications, all_papers_dict, all_embeddings, author_embeddings, all_author_ids
                )
